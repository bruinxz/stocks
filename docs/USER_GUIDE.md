...
“A 股早报”“高倍潜力”和“研究闭环模拟盘”会同时显示策略准入状态。只有带完整 PIT、至少 252 个样本外交易日、walk-forward PASS、正的基准超额、最大回撤不超过 20%、过拟合风险不超过 0.3，且双倍成本压力下仍为正收益的研究源，才能新增模拟仓位。`FAIL` 或 `INSUFFICIENT` 不是页面故障：系统会停止买入，已有模拟持仓只允许减仓退出。

候选策略评估默认只读，不会自动晋级未物化参数：

```bash
python3 scripts/ops/evaluate_research_strategy_candidates.py \
  --env-file backend/.env
```

只有候选、PIT 快照和证据哈希完全一致时，带 `--write-audit` 的结果才可能产生 `PASS` 审计；新发现的权重或持仓宽度必须先独立物化和复核，不能直接解锁模拟交易。
