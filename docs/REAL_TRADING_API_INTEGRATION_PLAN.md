...
> QMT / PTrade 的下一步落地拆解见：[QMT / PTrade 实盘交易接入下一步方案](QMT_PTRADE_LIVE_TRADING_ROADMAP.md)。
...
1. **先补产品边界**：页面、文档、环境变量都明确当前是模拟盘/只读实盘辅助，不是无人代操。
2. **先接真实行情**：让所有推荐和飞书消息里的价格、时间戳、行情延迟更真实。
3. **再做券商只读**：同步真实账户但不交易，并完成“真实账户 vs 策略模拟盘”的日常对账。
4. **再做订单草稿**：系统建议，用户确认；对账里“仅模拟建议”的股票可以进入草稿候选，但仍必须重新过行情 SLA、资金和仓位风控。
...
## 14. 当前落地进度（2026-05-22）

已完成第一阶段安全底座：

- 后端已新增 `backend/src/live-trading/` 独立模块，包含券商网关、行情 provider、实盘风控、安全边界、订单草稿、审批和审计链路。
- 默认 `mock_guarded` 网关永不下单；`env_readonly` 网关只从环境变量读取账户/持仓样例，也永不下单。
- `GET /api/live-trading/readiness` 已返回安全开关、券商能力、行情 SLA 与 provider 对比。
- `POST /api/live-trading/order-drafts/:id/approve` 已加入强确认与提交前二次行情/账户/风控复核；默认环境仍会阻断真实提交。
- 前端已新增「实盘交易」模块，包含：
  - `/live-trading`：安全边界、券商与行情网关、订单草稿；
  - `/live-trading/reconcile`：只读对账视图（复用实盘页）；
  - `/live-trading/orders`：订单审批入口（复用实盘页）。
- 只读对账已完成 MVP：
  - 对比券商只读快照与五套策略模拟账户；
  - 输出对齐分、快照年龄、仅实盘持仓、仅模拟建议、平均权重差；
  - 单票展示实盘市值/权重、模拟市值/权重和偏离标签；
  - 仅给出建议，不自动补仓/卖出。
- 策略候选转草稿已完成 MVP：
  - `GET /api/live-trading/order-draft-candidates` 输出「仅模拟建议 / 实盘偏轻」候选；
  - `POST /api/live-trading/order-drafts/from-candidate` 只生成实盘订单草稿，不提交券商；
  - 生成前要求券商只读资产快照、可用行情、100 股整手、未重复草稿；
  - 草稿仍会经过基础实盘风控，确认前还会二次复核。

下一阶段建议：

1. 接真实授权行情源，把 `LIVE_LICENSED_QUOTE_URL_TEMPLATE` 和字段映射配置到生产环境。
2. 接真实券商只读网关，先每日收盘同步账户、持仓、委托、成交，并跑对账报告。
3. 接入真实券商只读网关后，让候选草稿使用真实可用资金/真实持仓进行复核，并沉淀每日对账报告。
4. 成交回报同步后，将真实成交收益纳入策略归因，但与模拟收益分开展示。
