...
- Canary 小流量：优先点击「Canary预览」，系统只选择少量高置信参数进入小流量观察；确认后点「启动Canary」。该操作只写入少量任务参数并记录 Canary 审计，不会立即触发买卖，后续用闭环交易数量、超额收益和胜率决定是否扩大。
- Canary 评审：小流量卡片会给出“建议扩大 / 建议回滚 / 继续观察 / 暂不扩大”的只读结论和评审分，依据闭环样本、观察天数、平均超额收益、胜率和利润因子生成；该结论不会自动修改参数，必须人工再走审计预览。
- Canary 归因与回滚：小流量卡片还会展示“收益归因”和“回滚预案”。收益归因回答本次小流量后闭环交易的超额收益、胜率和贡献/拖累股票；回滚预案展示如果要恢复到 Canary 前，会影响几个任务、几个参数，以及是否存在后续被其它流程改过的参数。需要回滚时必须先点「回滚预览」，确认预案状态为“可回滚”后再点「确认回滚」，并输入 `CONFIRM_CANARY_ROLLBACK` 强确认；系统才会恢复任务参数、写入审计日志并重新加载任务，不会立即触发买卖。
...
- 路径：`/live-trading`、`/live-trading/reconcile`、`/live-trading/orders`
- 导航：实盘交易 → 安全边界 / 只读对账 / 订单审批
- 主要用途：在接入真实行情和券商 API 前，先确认系统处于什么实盘能力状态。当前默认是安全禁用或只读观察，不会提交真实委托。

页面重点：

1. 查看 `LIVE_TRADING_ENABLED`、`LIVE_ORDER_EXECUTION_ENABLED`、`LIVE_TRADING_KILL_SWITCH` 等开关形成的最终安全状态。
2. 查看券商网关是否仍为 `mock_guarded` 安全占位网关。
3. 查看行情 provider 是否仍为本地缓存，商业化前需替换授权实时行情源。
4. 可创建“实盘订单草稿”，但草稿只用于审批链路和风控解释演练。
5. 强确认文本为 `CONFIRM_LIVE_ORDER`；默认环境即使输入强确认，也会被后端安全边界阻断，不会真实下单。
6. 行情 SLA 会展示当前 provider、检查样本数、缺失/延迟数量、最大延迟和是否具备对外授权；订单草稿会把行情缺失、延迟、非实时、限价偏离纳入风控清单。
   - 页面还会展示“本地行情缓存 vs 可配置授权行情源”的 provider 对比；未配置 `LIVE_LICENSED_QUOTE_URL_TEMPLATE` 时授权源显示为“未配置”，不会影响默认安全边界。
   - 若设置 `LIVE_BROKER_GATEWAY=env_readonly`，系统只会从环境变量读取账户/持仓样例用于只读对账，仍不支持真实下单。
7. “只读对账”会把券商只读快照与五套策略模拟账户做横向对比，展示：
   - 对齐分、快照是否过期、仅实盘持仓、仅模拟建议、平均权重差；
   - 各策略模拟账户的总资产、持仓数、暴露比例；
   - 单票层面的实盘市值/权重、模拟市值/权重和差异标签。
   该模块只用于发现“真实账户是否偏离系统纪律”，不会自动补仓或卖出。
8. “策略候选生成实盘草稿”会把只读对账里「仅模拟建议 / 实盘偏轻」的股票筛成候选：
   - 必须已有券商只读资产快照；
   - 必须有可用行情和 100 股整手数量；
   - 已有未完成草稿的股票会自动去重；
   - 点击「生成草稿」只创建订单草稿，不会真实下单。
9. 点击确认前后端会再次执行行情/账户/风控二次复核，复核不过会阻断提交并写入审计。

操作建议：

- 开发/联调阶段只验证草稿、风控和审计，不连接真实券商交易权限。
- 真正接入券商前，先启用只读同步，确认资产、持仓、委托、成交回报完全一致。
- 如果对账提示“仅实盘持仓”较多，应优先补齐这些仓位的买入依据、止损线和退出计划；如果提示“仅模拟建议”较多，只能作为订单草稿候选，仍需行情 SLA 与人工确认。
- 页面上的策略候选适合用于“把模拟盘纪律翻译成待确认订单”，不代表系统建议直接买入；若没有券商只读资产快照，草稿会被风控阻断。
- 任何真实下单能力必须同时满足环境开关、券商非 Mock 网关、熔断关闭、用户强确认和风控通过。
