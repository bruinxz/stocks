...
```bash
# 查最新一份服务端备份
ssh ops@103.242.3.87 'stat /backup/stocks/postgres/latest.dump | head -5'
...
# 看服务端 cron 是否在跑
ssh ops@103.242.3.87 'tail -20 /var/log/stocks/pg_backup_daily.log'
```

## 关联
- crp (`college-recommendation-platform`) 的同款运维文档在 `crp 项目的 docs/operations/`
- 两个项目共用同一台香港服务器 (`103.242.3.87`)
