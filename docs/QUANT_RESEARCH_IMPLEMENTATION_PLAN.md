...
- [ ] 部署后发起 3 组量化跑分任务：趋势动量、均值回归/突破、多因子/风控/量价确认；观察任务队列、PM2 日志和服务器负载。
- [ ] 部署后确认「量化策略开盘机会扫描」定时任务存在且启用，明天 09:35 可刷新实时行情、生成量化推荐、提交 Agent 复核并写入飞书。
- [ ] 部署后补一轮 smoke，确认量化排行榜、跑分列表、调度任务与飞书消息链路没有断。
