...
- **内部高频数据接口**：全市场与历史数据统一调用同机 stocks 后端，默认 `http://127.0.0.1:3000`。通过 `tradingagents/dataflows/internal_api.py` 的 `InternalStockAPI` 访问 `GET /api/internal/stocks` 与 `POST /api/internal/data/batch-history`；运行时必须注入与后端一致的 `INTERNAL_API_KEY`。
