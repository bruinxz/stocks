...
INSERT INTO multibagger_text_hit (
  multibagger_universe_id, market_scope, ticker, source_document_id,
  source_version, document_fact_hash, taxonomy_version, term_id, hit_kind,
  field, start_offset, end_offset, evidence_ref, effective_at_utc,
  available_at_utc, fact_hash
) SELECT multibagger_universe_id, 'cn_a', ticker, '600000:2026-07-10',
         'v1', repeat('f', 64), 'taxonomy-v1', 'capacity_expansion',
         'EARLY_NEWS', 'TITLE', 3, 9, 'fixture#L1',
         effective_at_utc, available_at_utc, repeat('1', 64)
...
BEGIN
  ...
  BEGIN
    INSERT INTO backtest_pit_snapshot (
      ...
      'empty-source', '2026-07-10T06:30:00Z', '2026-07-10', TRUE,
      '{}', '{}', '{}', repeat('4', 64)
    );
  ...
  BEGIN
    INSERT INTO backtest_pit_snapshot (
      ...
      'missing-evidence', '2026-07-10T06:30:00Z', '2026-07-10', FALSE,
  ...
  BEGIN
    INSERT INTO multibagger_universe (
      market_scope, exchange, ticker, record_kind, universe_source_kind,
      source_document_id, source_version, effective_at_utc, available_at_utc,
      as_of_utc, is_publishable_candidate, fact_hash
    ) VALUES (
      'us', 'ACADEMIC_REFERENCE', '__AGGREGATE__:BAD', 'FRENCH_AGGREGATE',
      'kenneth_french', 'bad', 'v1', '2026-06-30T00:00:00Z',
      '2026-07-01T00:00:00Z', '2026-07-01T00:00:00Z', TRUE, repeat('6', 64)
    );
    RAISE EXCEPTION 'expected French publication check violation';
  EXCEPTION WHEN check_violation THEN NULL;
  END;
  ...
  BEGIN
    INSERT INTO multibagger_text_hit (
      ...
      field, start_offset, end_offset, evidence_ref, effective_at_utc,
      available_at_utc, fact_hash
    ) SELECT multibagger_universe_id, 'cn_a', ticker, 'duplicate', 'v2',
             repeat('f', 64), 'taxonomy-v1', 'capacity_expansion',
             'EARLY_NEWS', 'TITLE', 3, 9, 'duplicate',
             effective_at_utc, available_at_utc, repeat('7', 64)
      FROM multibagger_universe
      WHERE universe_source_kind = 'baostock_cn' AND record_kind = 'DAILY';
  ...
  BEGIN
    INSERT INTO jpkr_fx_observation (
      pair, direction, observation_day, effective_at_utc, available_at_utc,
