...
// 注册后总数 = 15 (8 base + liquidity + analyst_consensus + quality_high + earnings_surprise + momentum_reversal + east_money_qa + shareholder_concentration)
{
  const names = factorRegistry.listNames();
  assert(`registry 共 15 个因子 (实际 ${names.length})`, names.length === 15, names.join(', '));
}
