...
export class LocalDataStore {
  ...
  public async getDailyBars(symbol: string, startDate?: Date, endDate?: Date): Promise<LocalDailyBar[]> {
  ...
  public async saveDailyBars(symbol: string, newBars: LocalDailyBar[]): Promise<void> {
    ...
    const mergedBars = Array.from(barsMap.values()).sort((a, b) => a.time.getTime() - b.time.getTime());
  ...
  public async addFavorite(favorite: Omit<LocalFavorite, 'id' | 'createdAt' | 'updatedAt'>): Promise<LocalFavorite> {
    const allFavorites = this.readJson<LocalFavorite>(this.favoritesFile);
    
    // Check if exists
    const existing = allFavorites.find(f => f.userId === favorite.userId && f.symbol === favorite.symbol);
    if (existing) {
