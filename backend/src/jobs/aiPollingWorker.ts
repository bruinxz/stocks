...
if (aiPollingWorkerDisabled) {
  ...
  aiPollingQueue.process(async (job: Job<AIPollingJobData>) => {
    const {
      ...
      agent_only_paper_trade_risk_profile_gate,
    ...
    try {
      ...
      if (status === 'COMPLETED') {
        ...
        const agentOnlyAutoTradeEnabled =
          agent_only_auto_paper_trade === undefined
            ? Boolean(auto_paper_trade)
            : Boolean(agent_only_auto_paper_trade);
        const shouldMirrorToAgentOnly =
          agentOnlyAutoTradeEnabled &&
          archivedSignal &&
          !Boolean(quant_agent_fusion || scheduler_task_type === 'QUANT_DAILY_PIPELINE') &&
          [AISignalDecision.BUY, AISignalDecision.STRONG_BUY].includes(
            archivedSignal.normalized_decision as any
          ) &&
          Number(archivedSignal.confidence_score || 0) >=
            Number(agent_only_paper_trade_min_score || paper_trade_min_score || 72);
          ...
          try {
            const agentOnlyResult = await paperTradingAutomationService.autoBuyFromSignals({
              ...
              max_positions: Number(
                agent_only_paper_trade_max_positions || paper_trade_max_positions || 8
              ),
              default_position_pct: Number(
                agent_only_paper_trade_default_position_pct || paper_trade_default_position_pct || 4
              ),
              max_position_pct: Number(
                agent_only_paper_trade_max_position_pct || paper_trade_max_position_pct || 8
              ),
              min_trade_amount: Number(
                agent_only_paper_trade_min_trade_amount || paper_trade_min_trade_amount || 3000
              ),
              risk_profile_gate:
                agent_only_paper_trade_risk_profile_gate || paper_trade_risk_profile_gate,
              external_environment_policy: {
                ...(environment_policy || {}),
                strategy_budget_discipline,
                agent_only_control_group: true,
              },
              allowed_risk_levels: ['low', 'medium'],
              require_action_buy: false,
              ignore_profit_gate_for_forced_signals: true,
            ...
            logger.info(
              `TradingAgents 独立对照盘跟单完成 ${symbol}: 成交 ${agentOnlyResult.executed}，跳过 ${agentOnlyResult.skipped}`
            );
            paperTradingResult = {
              ...(paperTradingResult || {}),
              agent_only: {
                portfolio_id: agentOnlyResult.portfolio_id,
                executed: agentOnlyResult.executed,
                planned: agentOnlyResult.planned,
                skipped: agentOnlyResult.skipped,
                trades: agentOnlyResult.trades,
              },
            };
          } catch (agentOnlyError: any) {
            logger.warn(`TradingAgents 独立对照盘跟单失败 ${symbol}: ${agentOnlyError.message}`);
          }
        }
        ...
        return {
          ...
          paper_trading: paperTradingResult
            ? {
                ...
                agent_only: paperTradingResult.agent_only,
              }
            : undefined,
        };
