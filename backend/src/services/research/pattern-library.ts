 ...
 */
function findPeaks(prices: number[], min_distance = 3): number[] {
  const peaks: number[] = [];
  for (let i = min_distance; i < prices.length - min_distance; i += 1) {
    let is_peak = true;
    for (let k = 1; k <= min_distance; k += 1) {
      if (prices[i] <= prices[i - k] || prices[i] <= prices[i + k]) {
        is_peak = false;
        break;
      }
    }
...
function findTroughs(prices: number[], min_distance = 3): number[] {
 ...
 */
export function detectHeadAndShouldersTop(prices: number[], lookback = 60): PatternResult {
 ...
 */
export function detectDoubleTop(prices: number[], lookback = 60): PatternResult {
  if (prices.length < lookback) return { detected: false, confidence: 0 };
  const slice = prices.slice(-lookback);
  const peaks = findPeaks(slice, 3);
  ...
  for (let a = 0; a < peaks.length - 1; a += 1) {
    for (let b = a + 1; b < peaks.length; b += 1) {
      const diff =
        Math.abs(slice[peaks[a]] - slice[peaks[b]]) / ((slice[peaks[a]] + slice[peaks[b]]) / 2);
      ...
      if ((peak_avg - valley) / peak_avg < 0.1) continue;
      return {
        detected: true,
 ...
 */
export function detectDoubleBottom(prices: number[], lookback = 60): PatternResult {
  if (prices.length < lookback) return { detected: false, confidence: 0 };
  const slice = prices.slice(-lookback);
  ...
  for (let a = 0; a < troughs.length - 1; a += 1) {
    for (let b = a + 1; b < troughs.length; b += 1) {
      const diff =
        Math.abs(slice[troughs[a]] - slice[troughs[b]]) /
        ((slice[troughs[a]] + slice[troughs[b]]) / 2);
      if (diff > 0.03) continue;
      ...
      if ((peak - trough_avg) / trough_avg < 0.1) continue;
      return {
        detected: true,
        confidence: 1 - diff / 0.03,
 ...
 */
export function detectRoundingBottom(prices: number[], min_length = 30): PatternResult {
  ...
  if (Math.abs(left - right) / left > 0.1) return { detected: false, confidence: 0 };
  const bottom = Math.min(...slice);
  const bottom_idx = slice.indexOf(bottom);
  // Bottom in middle 50%
  if (bottom_idx < slice.length * 0.3 || bottom_idx > slice.length * 0.7)
    return { detected: false, confidence: 0 };
  // Depth 12-50%
  const depth = (Math.min(left, right) - bottom) / Math.min(left, right);
  if (depth < 0.12 || depth > 0.5) return { detected: false, confidence: 0 };
  // Smoothness check: differential should not have big jumps
  let max_jump = 0;
  for (let i = 1; i < slice.length; i += 1)
    max_jump = Math.max(max_jump, Math.abs(slice[i] - slice[i - 1]) / slice[i - 1]);
  if (max_jump > 0.1) return { detected: false, confidence: 0 };
  return {
    detected: true,
    confidence: 1 - max_jump / 0.1,
 ...
 */
export function detectSymmetricalTriangle(prices: number[], lookback = 30): PatternResult {
  if (prices.length < lookback) return { detected: false, confidence: 0 };
  const slice = prices.slice(-lookback);
  ...
  return {
    ...
    pivot_indices: [
      ...
      troughs[troughs.length - 2],
      troughs[troughs.length - 1],
    ],
 ...
 */
export function detectAscendingTriangle(prices: number[], lookback = 30): PatternResult {
  if (prices.length < lookback) return { detected: false, confidence: 0 };
  const slice = prices.slice(-lookback);
  const peaks = findPeaks(slice, 2);
  const troughs = findTroughs(slice, 2);
  ...
  const peak_spread =
    (Math.max(...peaks.map(i => slice[i])) - Math.min(...peaks.map(i => slice[i]))) / peak_avg;
  if (peak_spread > 0.03) return { detected: false, confidence: 0 };
  // Troughs rising
  if (slice[troughs[troughs.length - 1]] <= slice[troughs[troughs.length - 2]])
    return { detected: false, confidence: 0 };
  return {
    detected: true,
    confidence: 0.7,
 ...
 */
export function detectDescendingTriangle(prices: number[], lookback = 30): PatternResult {
  if (prices.length < lookback) return { detected: false, confidence: 0 };
  const slice = prices.slice(-lookback);
  const peaks = findPeaks(slice, 2);
  const troughs = findTroughs(slice, 2);
  ...
  const trough_spread =
    (Math.max(...troughs.map(i => slice[i])) - Math.min(...troughs.map(i => slice[i]))) /
    trough_avg;
  if (trough_spread > 0.03) return { detected: false, confidence: 0 };
  if (slice[peaks[peaks.length - 1]] >= slice[peaks[peaks.length - 2]])
    return { detected: false, confidence: 0 };
  return {
    detected: true,
    confidence: 0.64,
 ...
 */
export function detectFallingWedge(prices: number[], lookback = 30): PatternResult {
  if (prices.length < lookback) return { detected: false, confidence: 0 };
  const slice = prices.slice(-lookback);
  const peaks = findPeaks(slice, 2);
  const troughs = findTroughs(slice, 2);
  ...
  const peak_slope =
    (slice[peaks[peaks.length - 1]] - slice[peaks[0]]) /
    Math.max(1, peaks[peaks.length - 1] - peaks[0]);
  const trough_slope =
    (slice[troughs[troughs.length - 1]] - slice[troughs[0]]) /
    Math.max(1, troughs[troughs.length - 1] - troughs[0]);
 ...
 */
export function detectRisingWedge(prices: number[], lookback = 30): PatternResult {
  if (prices.length < lookback) return { detected: false, confidence: 0 };
  const slice = prices.slice(-lookback);
  const peaks = findPeaks(slice, 2);
  const troughs = findTroughs(slice, 2);
...
export function detectBullishFlag(
  ...
  const flag_high = Math.max(...flag),
    flag_low = Math.min(...flag);
 ...
 */
export function detectBearishFlag(
  prices: number[],
  pole_length = 5,
  flag_length = 10
): PatternResult {
 ...
 */
export function detectBullishPennant(
  prices: number[],
  pole_length = 5,
  pennant_length = 10
): PatternResult {
 ...
 */
export function inferLocalRegime(
  closes: number[],
  lookback = 60
  ...
  if (vol_annual > 0.3) return 'volatile';
  if (ret > 0.1 && vol_annual < 0.2) return 'bull';
  if (ret < -0.1 && vol_annual < 0.2) return 'bear';
  return 'range';
}
...
export const PATTERN_REGIME_CROSS_TABLE: Record<string, Record<string, number>> = {
  'Inverse Head and Shoulders': { bull: 0.85, bear: 0.75, range: 0.8, volatile: 0.7 },
  'Head and Shoulders Top': { bull: 0.72, bear: 0.85, range: 0.78, volatile: 0.65 },
  'Triple Bottom': { bull: 0.82, bear: 0.65, range: 0.75, volatile: 0.6 },
  'Triple Top': { bull: 0.6, bear: 0.78, range: 0.7, volatile: 0.55 },
  'Double Bottom': { bull: 0.78, bear: 0.65, range: 0.72, volatile: 0.6 },
  'Double Top': { bull: 0.62, bear: 0.74, range: 0.68, volatile: 0.55 },
  'Cup with Handle': { bull: 0.8, bear: 0.55, range: 0.68, volatile: 0.5 },
  'Rounding Bottom': { bull: 0.76, bear: 0.62, range: 0.7, volatile: 0.55 },
  ...
  'Rising Wedge': { bull: 0.58, bear: 0.7, range: 0.65, volatile: 0.55 },
  'Bullish Flag': { bull: 0.78, bear: 0.45, range: 0.55, volatile: 0.5 },
  'Bearish Flag': { bull: 0.45, bear: 0.72, range: 0.55, volatile: 0.5 },
  'Bullish Pennant': { bull: 0.7, bear: 0.4, range: 0.5, volatile: 0.45 },
};
 ...
 */
export function vcpPatternMultiplier(
  prices: number[],
  regime: string
): {
  multiplier: number;
  ...
  const checks: Array<{ name: string; fn: () => PatternResult }> = [
    {
      ...
      fn: () => ({ detected: false, confidence: 0 }) /* (Sprint 13 has it) */,
    },
  ...
  const multiplier = max_success > 0 ? 0.5 + ((max_success - 0.5) / 0.35) * 1.0 : 1.0;
 ...
 */
export function donchianBreakoutWithPatternAdjustment(
  prices: number[],
  regime: string,
  breakout_window = 20
): {
 ...
 */
export function turtleEntryWithPatternFilter(
  prices: number[],
  regime: string
): {
  ...
  if (prices.length < 56) {
    return {
      ...
      high55: 0,
      stop_low20: 0,
      pattern_validation: { multiplier: 1, detected_patterns: [] },
      proceed: false,
    };
  }
