...
export class AISignalController {
  syncFromScreeners = async (req: Request, res: Response) => {
    try {
      ...
      const verification = req.query.verify !== 'false' ? await aiInvestmentSignalService.verifySignals({ limit: 200 }) : null;
  ...
  listSignals = async (req: Request, res: Response) => {
    try {
      const { symbol, decision, source_type, start_date, end_date, limit = '50', offset = '0' } = req.query;
  ...
  verifySignals = async (req: Request, res: Response) => {
    try {
      ...
      const result = await aiInvestmentSignalService.verifySignals({ limit });
