...
const StockDetailPanel: React.FC<Props> = ({
  symbol: rawSymbol,
  showBack = false,
  onBack,
  showHeader = true,
    ...
    return {
      ...
      series: [
        ...
        },
        {
          ...
          smooth: true,
          showSymbol: false,
          lineStyle: { width: 1, color: '#7f8c8d' },
        },
        {
          name: '成交量',
          type: 'bar',
    ...
    <div style={{ padding: compact ? 0 : 16 }}>
      {showHeader && (
        <Space style={{ marginBottom: 16 }} wrap>
          ...
          <Button icon={<RobotOutlined />} type="primary" onClick={() => setAiOpen(true)}>
            AI 解读
          </Button>
        </Space>
      )}
        ...
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic
              ...
              valueStyle={{
                color:
                  (lastBar ? changePct : Number(stockInfo?.change_percent ?? 0)) >= 0
                    ? '#dc2626'
                    : '#16a34a',
                fontSize: compact ? 18 : 22,
              }}
          ...
          <Card size="small">
            ...
            {summary?.priceChange && (
              <div style={{ marginTop: 4, fontSize: 12 }}>
                ...
                <Text
                  strong
                  style={{ color: summary.priceChange.startsWith('-') ? '#16a34a' : '#dc2626' }}
                >
                  {summary.priceChange}
                </Text>
              </div>
              ...
              children: stockInfo ? (
                <Descriptions bordered column={compact ? 1 : 2} size="small">
                  ...
                  <Descriptions.Item label="总市值">
                    {stockInfo.total_market_cap
                      ? `¥${(Number(stockInfo.total_market_cap) / 1e8).toFixed(2)} 亿`
                      : '—'}
                  </Descriptions.Item>
                  <Descriptions.Item label="流通市值">
                    {stockInfo.circulating_market_cap
                      ? `¥${(Number(stockInfo.circulating_market_cap) / 1e8).toFixed(2)} 亿`
                      : '—'}
                  </Descriptions.Item>
                  <Descriptions.Item label="动态 PE">
                    {stockInfo.pe_dynamic || '—'}
                  </Descriptions.Item>
                ...
                <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      align: 'right',
                      render: (v: number) => (
                        <Text style={{ color: v >= 0 ? '#dc2626' : '#16a34a' }}>
                          {v >= 0 ? '+' : ''}
                          {v?.toFixed(2)}%
                        </Text>
                      ),
              ...
              children: factorsLoading ? (
                ...
                <>
                  <div style={{ marginBottom: 8, fontSize: 12, color: '#999' }}>
                    trade_date: <Tag color="blue">{factorsTradeDate || '—'}</Tag>共 {factors.length}{' '}
                    个因子（按 |z_score| 降序）
                  </div>
                  <Table
                    size="small"
                    ...
                    columns={[
                      ...
                      },
                      {
                        ...
                        render: (v: number | null) => {
                          ...
                          const bg =
                            v >= 0
                              ? `rgba(207, 19, 34, ${0.1 + intensity * 0.5})`
                              : `rgba(20, 177, 67, ${0.1 + intensity * 0.5})`;
                          return (
                            <div
                              style={{
                                background: bg,
                                ...
                                borderRadius: 3,
                                textAlign: 'right',
                                fontWeight: Math.abs(v) > 1 ? 600 : 400,
                              }}
                            >
                              {v >= 0 ? '+' : ''}
                              {v.toFixed(2)}
                      ...
                      },
                      {
                        ...
                        render: (v: number | null) => {
                          ...
                          const pct = v * 100;
                          return (
                            <Text style={{ color: pct >= 50 ? '#dc2626' : '#16a34a' }}>
                              {pct.toFixed(1)}%
                            </Text>
                          );
                        },
                  ...
                  <div style={{ marginTop: 12, fontSize: 11, color: '#999' }}>
                    说明：z_score 横截面标准化 (整个 A 股池). |z| &gt; 1.5 = 极端值. 红色 =
                    高于均值, 绿色 = 低于均值.
                  </div>
                </>
              ),
                ...
                <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      render: (v: string) => {
                        const color =
                          RECOMMENDATION_COLORS[v as keyof typeof RECOMMENDATION_COLORS] ||
                          'default';
                        const label =
                          RECOMMENDATION_LABELS[v as keyof typeof RECOMMENDATION_LABELS] || v;
                    ...
                    },
                    {
                      ...
                      width: 120,
                      render: (v: string[]) =>
                        v && v.length ? <Text style={{ fontSize: 11 }}>{v.join(', ')}</Text> : '—',
                    },
                    {
                      title: '摘要',
                      dataIndex: 'summary',
