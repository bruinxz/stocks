...
export function MorningBriefTable({ data, loading, onRowClick, selectedSymbol }: Props) {
  const columns: ColumnsType<CandidateListEntry> = [
      ...
      render: (_, r) => {
        ...
        return (
          <span style={{ fontFamily: 'var(--cd-font-mono)', fontWeight: 600 }}>
            {r.score?.score?.toFixed(1) ?? '--'}{band ? ` ${band}` : ''}
          </span>
        );
      },
    },
    {
      title: '催化来源', key: 'catalystSource', width: 120, ellipsis: true,
      render: (_, r) => r.latest_catalyst?.title ?? '--',
    },
    {
      title: '催化类型', key: 'catalystKind', width: 100,
      render: (_, r) => {
    ...
    {
      title: '确信度', key: 'conviction', width: 80,
    ...
    {
      title: '入场区间', key: 'entry', width: 120,
      render: (_, r) => {
    ...
    },
    {
      ...
      render: () => <button type="button" style={{ color: 'var(--cd-accent)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>详情</button>,
    },
  ];
